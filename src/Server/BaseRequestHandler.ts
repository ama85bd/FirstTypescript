...
export abstract class BaseRequestHandler {
  ...
  protected async respondBadRequest(message: string) {
    this.res.statusCode = HTTP_CODES.BAD_REQUEST;
    this.res.write(message);
  }

  protected async respondUnauthorized(message: string) {
    this.res.statusCode = HTTP_CODES.UNAUTHORIZED;
    this.res.write(message);
  }

  protected async respondText(code: HTTP_CODES, message: string) {
